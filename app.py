...
# from modules.unsplit import unsplit_model
#
# app = Flask(__name__)
#
# AVAILABLE_MODELS = {
#     "xsum": "./models/pythia70m-xsum/",
#     "mlsum": "./models/pythia70m-mlsum/",
...
# Liste des modèles disponibles et leurs dossiers
AVAILABLE_MODELS = {
    "xsum": "./models/pythia70m-xsum-100p-trained/",
    "mlsum": "./models/pythia70m-french-mlsum-adapted/",
}
...
def summarize():
    ...
    text = data.get("textToSum", "")
    optimized = data.get("optimized", True)
    model_key = data.get("model", "xsum")  # <- clé du modèle choisi

    if model_key not in AVAILABLE_MODELS:
        return jsonify({"error": f"Modèle inconnu : {model_key}", "success": False}), 400

    model_folder = AVAILABLE_MODELS[model_key]
    ...
    try:
        ...
        return jsonify({
            ...
            "mode": "Optimisé" if optimized else "Non-optimisé",
            "results": result,
            "metadata": {"input_length": len(text), "model": model_key}
        }), 200
    except Exception as e:
...
def compare():
    ...
    try:
        ...
        energy_gain = round(((result_non_opt['energy_consumed'] - result_opt['energy_consumed']) / result_non_opt['energy_consumed']) * 100, 2) if result_non_opt['energy_consumed'] > 0 else 0
